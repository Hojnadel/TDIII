...
texto_T2:
	dw MSG_ROW+SCREEN_WIDTH*5+MSG_COL
...
text_hexa_prefix_T2:
	dw HEXA_PREFIX_COL+SCREEN_WIDTH*5+HEXA_PREFIX_ROW
	db "0x ",0
...
print_screen:	
	...
	lea edx,[text_hexa_prefix_T1]		;cargo el texto 
	push edx						;pusheo dirección de inicio del texto
...
print_screen_T2:	
	...
	lea edx,[texto_T2] 				;cargo el texto 
	push edx						;pusheo dirección de inicio del texto
	push dword TEXT_COLOR			;pusheo el color del texto
	call show_text
	pop eax
	...
	lea edx,[text_hexa_prefix_T2]		;cargo el texto 
	push edx						;pusheo dirección de inicio del texto
	push dword NUMBER_COLOR			;pusheo el color del texto
	call show_text
	pop eax
	...
	.show_acum_sum:
		mov edi, NUMB_COL+SCREEN_WIDTH*5+NUMB_ROW	;cargo la posición en la pantalla del numero a mostrar
		add	edi, __VIDEO_BUFFER_START 	;ubico esa posición en memoria de video
		mov ecx, 0x8				;Recorrer un quad word, 8 bytes
		mov esi, acum_sum_T2+7			;Empiezo por el byte más significativo del acumulador
		.show_number_loop:
			cmp ecx,4				;si estoy en la mitad del numero, pongo un separador para leer más facil
			jnz	.not_put_separator
			inc edi 
...
print_screen_T1:	
	...
	lea edx,[texto_T1] 				;cargo el texto 
	push edx						;pusheo dirección de inicio del texto
	push dword TEXT_COLOR			;pusheo el color del texto
	call show_text
	pop eax
