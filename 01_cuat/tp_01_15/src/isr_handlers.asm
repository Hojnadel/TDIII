...
EXTERN SIMD_CONTEXT_T1

	EX7_NM_HANDLER:  		; Dev not available
		pushad
		clts
		FXRSTOR [SIMD_CONTEXT_T1]	;El contexto de SIMD de T1 y T2 comparten la dirección lineal
		popad
		iret
...
GLOBAL FLAG_TASK0
	...
	ISR0_0x20_HANDLER:	
		...
		;Checkeo si debo guardar SIMD
		push eax
		mov eax, cr0 						;cargo el cr0 para ver el TS
		and eax, 1<<3 						;bit TS
		jnz .save_context 					;si es '1' , salto
		fxsave [SIMD_CONTEXT_T1] 			;guardo el contexto (el contexto de SIMD de T1 y T2 comparten la dirección lineal)
		;Pongo el TS a 1
		mov eax, cr0
  		or eax, 0x1 <<3 					;CR0.TS en 1
  		mov cr0, eax
